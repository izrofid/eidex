...
export const parseEvolutions: Record<number, (evo: number[]) => string> = {
  ...
  25: (evo) => `Lvl in ${evo[2]}`,
  26: (evo) => `${getItemName(evo[2])} on male`,
  27: (evo) => `${getItemName(evo[2])} on female`,
  ...
  41: (evo) => `Lvl, with ${evo[2]}`,
  42: (evo) => `Lvl ${evo[2]} during fog in the overworld`,
  43: (evo) =>
    `Lvl, knows ${getMoveName(evo[2])} and (PID % 100) is 0`,
  44: (evo) =>
    `Lvl, knows ${getMoveName(evo[2])},  (PID % 100) is not 0`,
  45: (evo) =>
  ...
  47: (evo) => `Lvl after using ${evo[2]} 20 times`,
  48: (evo) => `Lvl after ${evo[2]}% recoil if male`,
  49: (evo) => `Lvl after ${evo[2]}% recoil if female`,
  50: (evo) => `Lvl after getting ${evo[2]} x999`,
  51: (evo) => `Lvl after KOing 3 of same species, holding ${evo[2]}`,
  52: (evo) => `Lvl after ${evo[2]} steps`,
};
...
export const parseShortEvolutions: Record<number, (evo: number[]) => string> = {
  ...
  21: (evo) => `Lvl, with ${getItemName(evo[2])} (Day)`,
  22: (evo) => `Lvl, with ${getItemName(evo[2])} (Night)`,
  23: (evo) => `Lvl, knowing ${getMoveName(evo[2])}`,
  24: (evo) =>
