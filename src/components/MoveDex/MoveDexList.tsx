...
export default function MoveList({ moves }: { moves: Move[] }) {
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(() => {
            if (
                ...
                setVisibleCount((prev) => Math.min(prev + 10, movesToDisplay.length));
            }
        }, 100);
    ...
    useEffect(() => {
        const handleScroll = () => {
            if (
                ...
                setVisibleCount((prev) => Math.min(prev + 50, movesToDisplay.length));
            }
        };
    ...
    return (
        <div className="flex w-full flex-1 select-none flex-col items-center">
            <div className="border-b-1 sticky top-0 z-10 w-full border-neutral-700 bg-zinc-800 p-3">
            </div>
